...
const roles = ["CETATEAN", "MODERATOR", "ADMINISTRATOR"];
...
const RoleDialog = ({ open, user, closeDialog }) => {
  const setDefaultZones = () => {
	if(localStorage.admin == "true") return ["COUNTY", "VILLAGE", "LOCALITY"];
    if (localStorage.zoneRoleOn == "COUNTY")
      return ["COUNTY", "VILLAGE", "LOCALITY"];
    if (localStorage.zoneRoleOn == "VILLAGE") return ["VILLAGE", "LOCALITY"];
    if (localStorage.zoneRoleOn == "LOCALITY") return ["LOCALITY"];
  };
  const [zones, setZones] = useState(setDefaultZones());
  ...
  const handleClose = (type) => {
    if (type == "save")
      closeDialog(user.id, zoneRole, zoneRoleOn, forcedUpdate);
    else closeDialog();
